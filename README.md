...
3. **Módulo de Compras (Abastecimiento):** Creación de facturación conectada al Kardex. Implementa cálculo dinámico de impuestos por ítem, inmutabilidad, y un "Botón de Pánico" que reversa automáticamente el costo promedio ponderado usando álgebra transaccional. Soporta compras intangibles omitiendo la afectación de stock.
4. **Módulo de Ventas (Validación NIIF y DIAN):** Configurado con lógica anti-negativos para bloquear ventas sin saldo físico. Expandido para soportar metadata DIAN (CUFE, XML). Incorpora un **Motor de Generación PDF** nativo (`xhtml2pdf`) para la representación gráfica de las facturas electrónicas.
